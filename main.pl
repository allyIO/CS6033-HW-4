% block(X): X is a block if it is a member of some blocks set BLOCKS
block(X):-
...
% notequal(X1, X2) takes two arguments and holds true when these arguments are 
...
substitute(X, Y, [X|T], [Y|T]).  /* Here, the head X of [X|T] is substituted by 
									...
									and NEW are the same, because we seek to 
									substitute only one occurrence. */

substitute(X, Y, [H|T], [H|T1]):- 
    substitute(X, Y, T, T1).  /* In this clause the element to be substituted is 
								NOT the head of the list, so the head H of the 
								list [H|T] carries over to the head H of the new 
								list, but the tail of the new list is obtained 
								from the tail T of the old list where the 
